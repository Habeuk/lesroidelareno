...
class FilterForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form = [
      ...
      'type_paragraph' => [
        '#type' => 'select',
        '#title' => $this->t('Type de paragraph'),
        '#options' => $this->getTypeParagraphs(),
        "#default_value" => $request->query->get("type_paragraph") ?? ""
      ],
      'submit' => [
        '#type' => 'submit',
  ...
  private function getTypeParagraphs() {
    $options = [
      '' => 'Aucun'
    ];
    foreach ($this->entityTypeManager->getStorage('paragraphs_type')->loadMultiple() as $id => $entity) {
      $options[$id] = $id . ' [ ' . $entity->label() . ' ]';
    }
    \ksort($options);
    return $options;
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $filter = [
      ...
      "type_paragraph" => $form_state->getValue("type_paragraph")
    ];
